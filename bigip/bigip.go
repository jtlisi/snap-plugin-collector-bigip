...
var baseNamespace = plugin.Namespace{plugin.NamespaceElement{Value: "jtlisi"}, plugin.NamespaceElement{Value: "bigip"}}
...
func (c F5Collector) GetConfigPolicy() (plugin.ConfigPolicy, error) {
	...
	cfg.AddNewStringRule([]string{"jtlisi", "bigip"}, "host", true)
	cfg.AddNewIntRule([]string{"jtlisi", "bigip"}, "port", false, plugin.SetDefaultInt(443), plugin.SetMinInt(0), plugin.SetMaxInt(65535))
	cfg.AddNewStringRule([]string{"jtlisi", "bigip"}, "username", false)
	cfg.AddNewStringRule([]string{"jtlisi", "bigip"}, "password", false)
	cfg.AddNewBoolRule([]string{"jtlisi", "bigip"}, "basic_auth", false, plugin.SetDefaultBool(false))
	cfg.AddNewBoolRule([]string{"jtlisi", "bigip"}, "https", false, plugin.SetDefaultBool(true))
	cfg.AddNewBoolRule([]string{"jtlisi", "bigip"}, "sanitize", false, plugin.SetDefaultBool(false))
	return *cfg, nil
}
